...
import com.yj.tech.common.code.BaseCode;
...
public class RabbitMQInitConfig implements CommandLineRunner {
    ...
    private void init(List<Object> clazzList) {
        ...
        if (ValidateUtils.isEmpty(clazzList)) {
            LogUtil.debug(getClass(), "MessageListenerConfig:没有rabbitMq消费者");
            return;
        }
    ...
    private void initDlx(Queue queue, Map<String, AbstractMQService> abstractMQMap) {
        ...
        if (void.class.equals(clazz)) {
            LogUtil.debug(getClass(), "MessageListenerConfig:initDlx 队列:{}不需要死信队列", queueName);
            return;
        }
        ...
        if (!AbstractConsumer.class.isAssignableFrom(clazz) || AbstractConsumer.class.equals(clazz)) {
            LogUtil.debug(getClass(), "MessageListenerConfig:队列:{}死信队列设置错误,死信队列类名为:{}", queueName, clazz.getName());
            return;
        }
        // 获取死信队列类
        ...
        LogUtil.debug(getClass(), "MessageListenerConfig队列:{}绑定{}死信队列", queue.getName(), queues.getName());
    }
