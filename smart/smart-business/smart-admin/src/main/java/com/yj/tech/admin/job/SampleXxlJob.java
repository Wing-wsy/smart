...
public class SampleXxlJob {
    ...
    public void shardingJobHandler() throws Exception {
        ...
        System.out.println("shardTotal=" + shardTotal + ",shardIndex=" + shardIndex + ",时间:" +new Date());
        Integer userMobilePlans = 0;

        if(shardTotal==1){
            //如果没有分片就直接查询所有数据
            userMobilePlans = 2000;
        }else{
            userMobilePlans = 1000;
        }
        System.out.println("处理任务数量:"+userMobilePlans);
        Long startTime = System.currentTimeMillis();
        for (int i = 0; i < userMobilePlans; i++) {
            try {
